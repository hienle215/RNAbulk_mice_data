...
# -------------------------
counts <- read_excel("C:\\Users\\leh\\OneDrive - TUNI.fi\\Desktop\\ISE LAB\\Ezh2_mouse_Markus_project\\20250423_Ezh2_Homo_GFP_LowvsWT_GFP_Low_deg.xlsx")
...
# -------------------------
# Step 3: Extract count columns (3–8)
# -------------------------
...
# -------------------------
# Step 5: Remove genes with all zero counts
# Remove rows with all zeros
count_matrix <- count_matrix[apply(count_matrix, 1, function(x) all(x > 0)), ]

# -------------------------
# Step 6: Sample metadata
# -------------------------
# Rename sample columns clearly
colnames(count_matrix) <- c("Homo1", "Homo2", "Homo3", "WT1", "WT2", "WT3")

colData <- data.frame(
  row.names = colnames(count_matrix),
  condition = c("Homo-GFP-low", "Homo-GFP-low", "Homo-GFP-low",
                "WT-GFP-low", "WT-GFP-low", "WT-GFP-low")
)

# -------------------------
# Step 7: Build DESeq2 dataset
# -------------------------
...
heatmap = pheatmap(mat_scaled,
                   ...
                   fontsize_col = 12) 
ggexport(heatmap, filename = "heatmap_DE_genes_hig_deg.tiff", height = 2000, width = 3000, res = 300)
...
p <- dotplot(ego, showCategory = 20, title = "GO Biological Process Enrichment")
# Zoom in on GeneRatio axis (y-axis in dotplot)
p + scale_x_continuous(limits = c(0.1, 0.8)) +
  theme_bw() +
  theme(
...
m_df <- msigdbr(species = "Homo sapiens", category = "H")  # "H" = Hallmark
...
### INF pathway calculation in RNAbulk
library(msigdbr)
library(dplyr)
...
ifn_score
group <- factor(c(rep("Homo-GFP-low",3), rep("WT-GFP-low",3)))  # adjust with your metadata
...
ggplot(df, aes(x = Group, y = IFN_score, fill = Group)) +
